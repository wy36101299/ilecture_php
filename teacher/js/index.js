...
$(document).on('click', '#question-box div.done', function(){
	$(this).hide().siblings('div.title').children('div').text('提出問題').end().parent().siblings('article[_que=1]').hide().end().siblings('[_que=0]').show();
});
...
function getMessagesHtml(mode, val, time, sId){
	if( mode === 'Mood' ){
		var html = '<div class="item mood" _sId='+sId+'>'+
			'<div class="">No.'+sId.split('_')[1]+'</div>'+
		...
		var html = '<div class="item" _sId='+sId+'>';
			html += ( sId === 'teacher' ) ? '<div class="">teacher</div><div class="ui segment" style="margin: 0;background: #38B1D8;color: white;">' : '<div class="">No.'+sId.split('_')[1]+'</div><div class="ui segment" style="margin: 0;">';
			html += '<p>'+val+'</p>'+
				'<div class="time">'+time+'</div>'+
			'</div>'+
		'</div>';
